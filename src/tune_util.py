...
def main(args):
    ...
    #     data_dir = os.path.join(data_dir, "generated_")

    ethics = any([name in args.dataset for name in ['cm', 'deontology', 'justice', 'virtue', 'util']])

    for run in range(args.nruns):
        model, optimizer = load_model(args)

        train_data = load_process_data(args, data_dir, args.dataset, "train", args.model)
        test_data = load_process_data(args, data_dir, args.dataset, "test", args.model)
        if ethics:
            test_hard_data = load_process_data(args, data_dir, args.dataset, "test_hard", args.model)
        ...
        with open("runs.txt", "a") as f:
            if ethics:
                ...
                f.write("Run {} test acc: {:.3f}\n".format(run, test_acc))

        if args.save:
            model_suffix = args.model.split("/")[-1] # discard organization and slash
            save_path = "../models/reward/{}_{}_{}_{}_{}.pkl".format(args.dataset, model_suffix, args.learning_rate, args.batch_size, args.nepochs)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", "-data", type=str, default="util")
    # parser.add_argument("--generated_data", "-gen", action="store_true", default=False, help="use generated scenarios labeled by a gold model")
    ...
    parser.add_argument("--filter_max_len", "-fm", action="store_true")
